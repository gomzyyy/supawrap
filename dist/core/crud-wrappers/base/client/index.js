 ...
 */
export class BaseClientCRUDWrapper extends UtilityMethods {
     ...
     */
    async createOne(data, cbs, opts = { allowFalsy: false }) {
     ...
     */
    async updateOneById(tableId, update, cbs, opts = { allowFalsy: false }) {
        return this.withLoading(cbs, async () => {
            try {
     ...
     */
    async batchUpdate(update, conditions, cbs) {
        return this.withLoading(cbs, async () => {
            try {
     ...
     */
    async get(getOptions, cbs) {
        return this.withLoading(cbs, async () => {
            try {
                const { limit, single, maybeSingle, sortBy, sort, search, searchFields, page = 1, offset, } = getOptions;
                let query = this.supabase
                    .from(this.tableName)
                    .select("*", { count: "exact" });
     ...
     */
    async deleteOneByIDPermanent(tableId, cbs) {
        return this.withLoading(cbs, async () => {
            try {
     ...
     */
    async toggleSoftDeleteOneById(tableId, intent, cbs) {
        return this.withLoading(cbs, async () => {
            try {
                if (!this.behaviour.supportsSoftDeletion) {
                    throw new ValidationError(`Table '${this.tableName}' doesn't support soft deletion.` +
                        "\n\n" +
